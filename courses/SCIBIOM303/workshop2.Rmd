...
We merge the two data sets together. We can do this by using the function `left_join()`, which enables us to merge all the columns from both data sets together. We join them by the columns 'Date' and 'Prefecture' which are the same in both data sets to avoid having double values. 
We then use the `mutate()` function to convert the Date variable to a proper date format with the `dmy()` function, which looks for a character string in a day-month-year format.
