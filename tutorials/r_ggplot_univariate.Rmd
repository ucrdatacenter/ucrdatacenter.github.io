...
Please watch [this](https://vimeo.com/1011773578/3fc4a9fa21) video (4:40), then read and follow along with the written tutorial below.
...
If you want R to automatically count the frequencies of your variable, use `geom_bar()` with only an `x` aesthetic.
If you have a separate variable that contains the frequencies (e.g. as the result of a `count()` function), you can use `geom_col()` with both an `x` and `y` aesthetic.
...
To learn more about other geoms and customization options, have a look at our [other tutorials](../) and [additional resources](../links#Help with `ggplot`).