...
This can be because you are only interested in a specific subset of observations or because you only need a subset of variables.
In this tutorial, we will show you how to filter rows and select columns from a tibble using `tidyverse` functions and the `diamonds` dataset (which comes pre-loaded with `tidyverse` so you don't need to import it).
