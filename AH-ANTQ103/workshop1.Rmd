...
R can also be used to assign values to vectors. Try the following operations. These vectors should be added to the environment pane (On the right)
Assign the values 1, 2, 3, 4, 5 to the vector x.
...
We can now use these vectors in calculations. Try the following operations. 

```{r vectors4, eval=FALSE}
x + y
x - y
x * y
x / y
...
Now we create a vector with a different length. Note that R will give a warning, but it will still perform the operation, reusing the values in the shorter vector.
...
We can also concatenate vectors together. Note that the order of the vectors and the order in the vectors matter
